...
def calculate_sum(sub_arr, result):
...
if __name__ == '__main__':
    ...
    for i in range(PARTS):
        ...
        thread = threading.Thread(target=calculate_sum, args=(sub_arr, result))
        thread.start()
    ...
    stop = time.time()
    print(f' Время работы {PARTS} потоков {round((stop - start),6)}')
